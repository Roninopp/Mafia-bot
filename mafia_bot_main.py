...
async def show_main_menu_callback(query, context):
    ...
    except Exception: # Fallback if not a photo
...
async def show_profile(query, context):
    ...
    try:
        ...
        await query.message.reply_text(profile_text, parse_mode='HTML', reply_markup=keyboard)
...
async def handle_target_practice(query, context, data):
    ...
    if action == 'Shoot':
        if target == 'Mafia':
            ...
            state['score'] -= 75
            feedback = " civilians!</b> -75 points!"
    elif action == 'Hold':
        if target == 'Mafia':
            state['score'] -= 50
...
async def handle_timed_disarm(query, context, data):
    ...
    if 'mission_state' not in context.user_data:
        # Game is over or expired, just ignore the click
        await query.answer()
        return
    ...
    # Game is still on, increment click
    state['clicks'] += 1
    await query.answer(f"Clicks: {state['clicks']}")
    
    # Only edit the message every 5 clicks to avoid rate limits
    if state['clicks'] % 5 == 0:
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update and update.effective_message:
        ...
        try:
            ...
            # Try to send user back to main menu
            await show_main_menu_callback(update.callback_query, context)
