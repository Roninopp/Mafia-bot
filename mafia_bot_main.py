...
from config import BOT_TOKEN, FEATURES, SHOP_ITEMS, ADMIN_IDS
...
# These functions have been MOVED to utils.py to prevent circular imports
# ---------------------------------
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # This function now comes from utils.py
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- FIX 2: Added handler for shop purchases ---
...
async def show_shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show shop items"""
    # --- FIX 2: Updated Shop Functionality ---
    user_id = update.effective_user.id
...
# --- FIX 2: Added function to handle purchases ---
async def handle_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """Handle item purchase logic"""
    user_id = update.effective_user.id
    player = player_manager.get_player(user_id)
    
    # Parse item name from button text
    # e.g., "💰 Buy: 🌟 Golden Skin (500)"
    try:
        item_name = text.split(':', 1)[1].split('(')[0].strip()
    except Exception as e:
        logger.error(f"Error parsing item name: {e}")
    ...
    if player_manager.has_item(user_id, item_to_buy['id']):
        await update.message.reply_text("✅ You already own this item!")
        return
    
    # Check for sufficient coins
    if player['coins'] < item_to_buy['price']:
        await update.message.reply_text(
            f"❌ Not enough coins! You need {item_to_buy['price']} coins.",
            parse_mode='HTML'
        )
        return
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TSTATYPE):
    """Quick stats command"""
    user_id = update.effective_user.id
    player = player_manager.get_player(user_id)
...
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"📈 Total Games: {len(game_manager.games.keys())}\n" # Corrected to show total created
    )
