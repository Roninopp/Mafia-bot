...
log_handler = logging.FileHandler('bot_activity.log') # New log file
...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"Callback: data='{data}', user={user_id}") # Log callback data
    ...
    try:
        ...
        logger.error(f"Err handling '{data}' for {user_id}: {e}", exc_info=True) # Log full traceback
...
async def handle_target_practice(query, context, data):
    ...
    target, action = state.get('current_target'), data.split('_')[-1]
    feedback = ""
    score = state.get('score', 0) # Get current score
    if action == 'Shoot':
