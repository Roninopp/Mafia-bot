...
class TradingSystem:
    ...
    def accept_trade(self, trade_id: str, accepting_user_id: int) -> Tuple[bool, str]:
        ...
        if sender['coins'] < trade['offer']['coins']:
            trade['status'] = 'failed'; return False, f"{trade['sender_username']} doesn't have enough coins!"
        for item_id in trade['offer']['items']:
            if not player_manager.has_item(trade['sender_id'], item_id):
                trade['status'] = 'failed'; return False, f"{trade['sender_username']} no longer owns the offered item: {item_id}"
        ...
        try:
            ...
            for item_id in trade['offer']['items']:
                ...
                if item_details:
                    ...
                    else: raise ValueError(f"Failed to remove {item_id} from sender {trade['sender_id']}")
                else: raise ValueError(f"Offered item {item_id} details not found")
            ...
            for item_id in trade['request']['items']:
                ...
                if item_details:
                    ...
                    else: raise ValueError(f"Failed to remove {item_id} from receiver {trade['receiver_id']}")
                else: raise ValueError(f"Requested item {item_id} details not found")
...
class TournamentSystem:
    ...
    def register_player(self, tournament_id: str, user_id: int, username: str) -> Tuple[bool, str]:
        ...
        if player_manager.spend_coins(user_id, tournament['entry_fee']):
            ...
            print(f"Player {username} registered for tournament {tournament_id}")
            return True, "Registered successfully!"
        else:
            return False, "Failed to process entry fee."

    def start_tournament(self, tournament_id: str) -> Tuple[bool, str]:
        """Start tournament and create brackets"""
        tournament = self.tournaments.get(tournament_id)
        if not tournament: return False, "Tournament not found!"
        if tournament['status'] != 'registration': return False, "Already started or finished!"
    ...
    def report_match_result(self, tournament_id: str, match_id: str, winner_user_id: int) -> Tuple[bool, str]:
        """Report match winner and advance brackets"""
        tournament = self.tournaments.get(tournament_id)
        if not tournament or tournament['status'] != 'in_progress': return False, "Tournament not active."

        brackets = tournament['brackets']
        current_round_index = brackets['current_round_index']
        if current_round_index >= len(brackets['rounds']): return False, "Invalid round index."
        ...
        for match in current_round_matches:
            if match['match_id'] == match_id and match['status'] != 'finished':
                ...
                print(f"Match {match_id} in {tournament_id}. Winner: {winner_data['username']}")
                break
        if not match_found: return False, "Match not found or finished."

        if all(m['status'] == 'finished' for m in current_round_matches):
            print(f"Round {current_round_index + 1} of {tournament_id} complete.")
            ...
            if len(winners) == 1:
                ...
                if winner_player:
                    ...
                    print(f"Tour {tournament_id} finished! Winner: {winner_player['username']}. Prize: {tournament['current_pot']}")
                    return True, f"Tournament finished! Winner: {winner_player['username']}"
                else: return False, "Winner data not found."
            elif len(winners) > 1:
                brackets['current_round_index'] += 1
                next_round_index = brackets['current_round_index']
                next_round_matches = []
                ...
                print(f"Created Round {next_round_index + 1} for {tournament_id}")
                return True, f"Round {current_round_index + 1} complete. Next round ready."
            else: return False, "Error advancing: No winners found?"
    ...
    def format_tournament_info(self, tournament_id: str) -> str:
        ...
        text = (f"🏆 <b>{tournament['name']}</b> 🏆\n\n🎮 Mode: {tournament['mode'].upper()}\n"
                ...
                f"💰 Entry Fee: {tournament['entry_fee']} coins\n🎁 Current Pot: {tournament['current_pot']} coins\n"
                f"📊 Status: {tournament['status'].upper()}\n")
        if tournament['status'] == 'finished' and tournament['winner']: text += f"🏅 Winner: {tournament['winner']['username']}\n"
    ...
    def format_tournament_brackets(self, tournament_id: str) -> str:
        ...
        for i, round_matches in enumerate(brackets['rounds']):
            ...
            for match in round_matches:
                p1_name, p2_name = match['player1']['username'], match['player2']['username']
            ...
            text += "\n"
        if tournament['status'] == 'finished' and tournament['winner']: text += f"🏅 <b>Winner: {tournament['winner']['username']}</b> 🏅\n"
        return text

# --- Seasonal Events & Clans remain unchanged ---
class SeasonalEvents: pass
class ClanSystem: pass

# --- Replay System Class Definition ---
class ReplaySystem:
    """Game replay system"""
    def __init__(self):
    ...
    def save_replay(self, game_id: str, game_data: dict):
        """Save game replay"""
        replay_data = game_data.copy() # Make a copy
        self.replays[game_id] = {
            'game_id': game_id,
            'mode': replay_data.get('mode', 'unknown'),
            'players': replay_data.get('players', []),
            'events': replay_data.get('events', []),
            'winner': replay_data.get('winner', None),
            'rounds': replay_data.get('round', 0),
            'saved_at': datetime.now().isoformat()
        }
        print(f"Replay saved for game {game_id}")
    ...
    def format_replay_summary(self, game_id: str) -> str:
        ...
        text = (f"🎬 <b>GAME REPLAY</b> 🎬\n\n🆔 Game ID: {game_id}\n🎯 Mode: {replay['mode'].upper()}\n"
                f"🔄 Rounds: {replay['rounds']}\n🏆 Winner: {str(replay['winner']).upper() if replay['winner'] else 'N/A'}\n\n<b>Players:</b>\n")
        for player in replay.get('players', []):
            status = "✅" if player.get('alive', False) else "💀"
            role = player.get('role', 'Unknown').upper()
            text += f"{status} {player.get('username', 'Unknown')} - {role}\n"
        return text

# --- Instantiate Global Systems HERE ---
trading_system = TradingSystem()
tournament_system = TournamentSystem()
replay_system = ReplaySystem() # This creates the instance needed by game_manager.py
# -------------------------------------
