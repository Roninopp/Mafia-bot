...
from config import EMOJIS, LOADING_FRAMES, ANIMATION_SEQUENCES
...
def format_night_summary(eliminated: dict, protected: dict, investigated: dict = None) -> str:
    """Format night phase summary"""
    text = "🌙 <b>NIGHT SUMMARY</b> 🌙\n\n"
    
    if eliminated and not protected.get('protected'):
        text += f"☠️ {eliminated['username']} was eliminated!\n"
        text += f"They were a <b>{eliminated['role'].upper()}</b>\n\n"
    elif protected.get('protected'):
