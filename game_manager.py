...
# --- FIX 4: Import replay system ---
from enhanced_features import replay_system

# --- FIX 1: Replaced wildcard import with specific imports ---
from utils import (
    ReplyKeyboardRemove, create_main_menu_keyboard, generate_game_id,
    send_phase_transition, create_player_action_keyboard,
    send_animated_message, send_elimination_animation,
    format_night_summary, check_game_end_condition,
    send_victory_animation, create_voting_keyboard,
    format_vote_results, format_day_summary
)
...
class GameManager:
    ...
    def create_game(self, mode: str, creator_id: int, chat_id: int) -> str:
        ...
        # Fallback if creator is somehow not found (should not happen with main bot logic)
    ...
    def join_game(self, game_id: str, user_id: int, username: str) -> Tuple[bool, str]:
        ...
        # --- FIX: Use correct max_players setting ---
    ...
    def _assign_roles(self, game: dict):
        ...
        # Ensure correct number of roles for players
    ...
    async def _process_night_actions(self, game_id: str, message, context):
            ...
            for player in game['players']:
                if player['user_id'] == kill_target:
                    if player.get('protected', False):
                        protected = True # The kill was blocked
                    else:
                        player['alive'] = False
                        eliminated_player = player
                    # player['protected'] = False # Reset is done at start of function
                    break
        ...
        if investigate_target:
            for player in game['players']:
                if player['user_id'] == investigate_target:
                    # --- FIX 4: Corrected investigation logic ---
                    ...
                    if player['role'] == 'godfather':
                        ...
                        investigation_result = {
                            'player': player,
                            # Check for 'mafia' team, not 'is_evil_role' (which includes Jester)
        ...
        if eliminated_player:
            ...
            # Pass the correct protection status to summary
    ...
    async def _process_day_votes(self, game_id: str, message, context):
        ...
        eliminated_player = None # Define variable outside 'if'
        ...
        if not votes:
            ...
            for voter_id, target_id in votes.items():
                if target_id == 'skip':
                    continue
                # Ensure target_id is int for dictionary key consistency
    ...
    async def _end_game(self, game_id: str, winner: str, message, context):
        ...
        game = self.get_game(game_id)
        if game['status'] == 'finished': # Avoid double-ending
            return
        ...
        for player in game['players']:
            if winner == 'villagers':
                if not role_manager.is_evil_role(player['role']):
                    winners.append(player)
                else:
                    losers.append(player)
            else:  # mafia wins
                if role_manager.is_evil_role(player['role']):
                    winners.append(player)
                else:
                    losers.append(player)
